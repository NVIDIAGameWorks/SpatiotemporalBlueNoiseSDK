#include "VectorBlueNoiseGaussianKernel.h"
...
size_t KernelRadius(float sigma, size_t maxKernelWidth)
{
    ...
    return std::min(radius, std::max((size_t)1,maxKernelWidth/2));
}

bool shouldUseEvenKernelWidth(float sigma, size_t maxKernelWidth)
{
    return KernelRadius(sigma, maxKernelWidth) * 2 > maxKernelWidth;
}

}

VectorBlueNoiseGaussianKernel::VectorBlueNoiseGaussianKernel(float sigma, size_t maxKernelWidth) : GaussianKernel(1.0f, 0.0f, sigma, KernelRadius(sigma, maxKernelWidth), shouldUseEvenKernelWidth(sigma, maxKernelWidth))
{
    operator[](0) = 1.0f;
    if (max() > 0)
    {

...
namespace _2Dx1Dx1D
{
    ...
    public:
        ...
        size_t c_dimsX = dimsX;
        size_t c_dimsY = dimsY;
        size_t c_dimsZ = dimsZ;
        size_t c_dimsW = dimsW;
        ...
        BlueNoiseTexturesND Make()
        {
            ...
            printf("creating 2Dx1Dx1D blue noise:  %zux%zux%zux%zu\n", dimsX, dimsY, dimsZ, dimsW);
            ...
            textures.Init({ dimsX, dimsY, dimsZ, dimsW }, { sigmaX, sigmaY, sigmaZ, sigmaW }, { 0, 0, 1, 2 });
    ...
    private:
        ...
        void SplatEnergy(size_t pixelIndex)
        {
            ...
            // splat the 1D gaussian along z
            {
                for (int iz = c_KernelRadiusStartZ; iz <= c_KernelRadiusEndZ; ++iz)
                {
                    ...
                    if (ON)
                        m_energy[pixelIndex] += m_kernelZ[abs(iz)];
                    else
                        m_energy[pixelIndex] -= m_kernelZ[abs(iz)];
                }
            }
            ...
            {
                for (int iw = c_KernelRadiusStartW; iw <= c_KernelRadiusEndW; ++iw)
                {
                    ...
                    if (ON)
                        m_energy[pixelIndex] += m_kernelW[abs(iw)];
                    else
                        m_energy[pixelIndex] -= m_kernelW[abs(iw)];
                }
            }
        ...
        void MakeInitialBinaryPattern()
        {
            ...
            // If we get duplicate random numbers, we won't get the full targetCount of on pixels, but that is ok.
            {
                ...
                {
                    size_t pixelIndex = pcg32_boundedrand_r(&rng, (int)c_numPixels - 1);
